...
const Planning: React.FC = () => {
  ...
  const toggleValidation = () => {
      ...
      if (newStatus === 'DRAFT') {
          if (!window.confirm("Attention : Déverrouiller le planning permet sa modification et sa régénération. Continuer ?")) {
              return;
          }
      } else {
          if (!window.confirm("Valider ce planning ?\n\nIl deviendra la version officielle et ne pourra plus être régénéré accidentellement.")) {
              return;
          }
      }
    ...
    <div className="space-y-4 print:space-y-4">
      ...
      <div className="glass-panel flex flex-col md:flex-row justify-between items-center gap-4 p-4 rounded print:hidden sticky top-0 z-20">
        ...
        <div className="flex gap-2">
            ...
            {schedule && (
                <span className={`inline-flex items-center px-3 py-1 rounded text-xs font-bold uppercase tracking-wider border backdrop-blur-sm ${
                    isValidated 
            ...
            {schedule && (
                <>
                    <button 
                        ...
                        className={`flex items-center px-4 py-2 text-xs font-bold uppercase tracking-widest rounded transition-all border shadow-lg ${
                            isValidated 
                            ? "bg-white/5 text-slate-400 hover:text-white border-white/10 hover:bg-white/10"
                            : "bg-green-600 hover:bg-green-500 text-white border-green-500"
                        }`}
                    >
                        {isValidated ? "DÉVERROUILLER" : "VALIDER"}
                    </button>

                    <button 
                        onClick={handlePrint}
                        className="p-2 bg-white/5 hover:bg-white/10 text-slate-300 border border-white/10 rounded transition-colors"
                        title="Imprimer"
                    >
